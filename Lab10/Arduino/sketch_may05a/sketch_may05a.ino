...
const byte ok_response[] = {ok_message,ok_message,ok_message};
...
void executeInterrupt(const byte income) {
    byte response;
    if (byteCount < 3) {
        ...
        if (Compute_CRC8(packet_from_slave) == income) {
            if (packet_from_slave[0] == 0xF0) {
                byteCount = 0;
                PORTC |= (1 << PC1);
                return;
            }
